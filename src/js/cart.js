require(["config"], function(){
	...
	if (_products.length === 0) { // 购物车为空
		$(".instru").html(`购物车为空，请<a href="http://localhost:8080/"  style="color:#891345">选购商品</a>`);
		$(".cart_title").detach();
		$(".city").detach();
		$(".box").detach();
		return;
	...
	$(".cart_body").on("click", ".add,.minus", function(){
		...
		// 显示小计
		_row.children(".sub").text(_prod.amount * _prod.price.substring(1));
		// 计算合计
		calcTotal();
	});
	// 输入修改
	...
	$("#shopping").click(function(){
		window.location.href = "http://localhost:8080/html/detail.html";
	});
